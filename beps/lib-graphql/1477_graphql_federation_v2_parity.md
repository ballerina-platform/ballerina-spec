...
- **Engine-native `_entities`/`_service` resolution.** Committed in and owned by the [Compile-Time-Resolved Dispatch BEP](1472_graphql_compile_time_dispatch.md) ([#1472](https://github.com/ballerina-platform/ballerina-spec/issues/1472)). This proposal depends on it and does not re-specify it — see [Dependencies](#dependencies).
...
- [Compile-Time-Resolved Dispatch BEP](1472_graphql_compile_time_dispatch.md) ([#1472](https://github.com/ballerina-platform/ballerina-spec/issues/1472)) — commits and owns the engine-native `_entities`/`_service` resolution and the entity type map this proposal builds on
