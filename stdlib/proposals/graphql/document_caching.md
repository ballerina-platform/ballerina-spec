...
This proposal is to introduce GraphQL document caching. The GraphQL document caching reduces the document parsing overhead which leads to improved performance.
...
After
```graphql
 querygetDetail($userID:Int!$searchTerm:String!$user:User!){user(id:$userId){id,name,email,role},posts{id,title},search(query:$searchTerm){...on User{id,name},...on Post{id,title}},usersByIds(input:$user){id,name,email}}
``` 
